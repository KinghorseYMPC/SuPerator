...
class ResearchOrchestrator:
    ...
    def __init__(self, cfg):
        ...
        self._code_missing_count = 0  # 追踪代码缺失次数，用于兜底
        self._fallback_used = False   # 记录是否使用了 code-ref 回退
    ...
    def should_stop(self) -> bool:
        ...
        if len(self.memory.experiments) >= self.cfg.research.early_stop_patience:
            ...
            best_score = self.memory.best_metrics.get("total", -1)
            no_improve = all(
                e.metrics.get("total", -1) <= best_score
                for e in recent
            )
    ...
    def run(self):
        ...
        while not self.should_stop():
            ...
            if not success:
                ...
                # 兜底机制：当设计阶段反复无法生成代码时，自动使用 code-ref 基线
                if self.memory.current_phase == "design" and not os.path.exists("code/train.py"):
                    self._code_missing_count += 1
                    if self._code_missing_count >= 3:
                        print("[Fallback] 多次代码生成失败，从 code-ref 复制基线代码到 code/ ...")
                        import shutil
                        if os.path.exists("code"):
                            shutil.rmtree("code")
                        shutil.copytree("code-ref", "code")
                        self._code_missing_count = 0
                        self._fallback_used = True
                        self.memory.current_phase = "experiment"
                        self.memory.code_versions.append({
                            "iteration": self.memory.iteration,
                            "timestamp": datetime.now().isoformat(),
                            "note": "Fallback: copied code-ref baseline due to repeated code generation failures.",
                        })
                        print("[Fallback] 基线代码就绪，将直接进入实验阶段。")
    ...
    def _generate_methodology_md(self, path: str) -> None:
        ...
        if self._fallback_used:
            lines.append("Code in the `code/` directory was autonomously generated by the LLM Agent, ")
            lines.append("with portions sourced from the reference baseline (code-ref/) when code generation ")
            lines.append("failed repeatedly. The generation process is documented in the task logs.")
        else:
            lines.append("All code in the `code/` directory was autonomously generated by the LLM Agent ")
            lines.append("during the research session. No human-written or pre-placed baseline code was used.")
