"""One-click Task 1 quick submission: train → parse → finalize → validate.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.validate_only:
        ...
        rc = _run([
            sys.executable,
            str(ROOT / "scripts" / "parse_pdeagent_task1_run.py"),
        ], "parse results")
        if rc != 0:
        ...
        if not args.skip_finalize:
            rc = _run([
                sys.executable,
                str(ROOT / "scripts" / "finalize_pdeagent_task1_submission.py"),
                "--validate",
            ], "finalize + validate")
            if rc != 0:
                return rc
    ...
    rc = _run([
        ...
        "--task-id", "1",
    ], "validate submission (task 1)")
    if rc != 0:
        return rc

    # Collect summary
    parsed_path = ROOT / "outputs" / "pdeagent_task1" / "parsed_quick_summary.json"
    ...
    print(f"\n[SUMMARY] Task 1 quick submission complete")
