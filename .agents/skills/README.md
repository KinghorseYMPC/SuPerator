...
- `local-first-compute`: local source-of-truth workflow for temporary SLURM or Kaggle compute backends, private backend config boundaries, explicit SLURM env types, manual remote minimal training boundaries, remote package plans, Kaggle package preparation guardrails, and artifact return checks.
