"""Task 1 training adapter for pdeagent-style baseline.

Adapted from pdeagent code-ref/train.py (external_references/).
Clean-room implementation — no import from external_references.

Provides a minimal training loop for ChunkedFNO1d on Task 1 Burgers data.
Uses PdeAgentTask1WindowDataset, PdeAgentTask1Model, and checkpointing.
"""
...
def train_one_epoch(
    ...
    """Train one epoch using MSE loss on chunk windows.

    Args:
        model: PdeAgentTask1Model instance.
        loader: DataLoader yielding {"input": (B,T,X), "target": (B,T,X)}.
...
def evaluate_one_step(
    ...
    """Evaluate one-step MSE without gradients.
...
def _load_checkpoint(path: str | Path, model: Any, device: str = "cpu") -> dict[str, Any]:
    import torch
...
def train_pdeagent_task1_baseline(config: dict[str, Any]) -> dict[str, Any]:
    """Train a pdeagent-style Task 1 baseline.

    Args:
        config: Dict with structure matching configs/pdeagent_task1_adapter_smoke.yaml.
    ...
    output_steps = int(model_config.get("output_steps", 10))
    ...
    train_indices = list(range(train_samples * win_per_traj))
    dev_start = train_samples * win_per_traj
    dev_indices = list(range(dev_start, dev_start + dev_samples * win_per_traj))
    ...
    dev_loader = DataLoader(
        Subset(full_dataset, dev_indices),
    ...
    result_dir = ROOT / output_config.get("result_dir", "outputs/pdeagent_task1")
